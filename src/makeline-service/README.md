...
## CosmosDB SDK repository details

The `makeline-service` uses `src/makeline-service/cosmosdb.go` to manage orders within Cosmos DB through the Azure Go SDK.

### Connection helpers
- `NewCosmosDBOrderRepoWithManagedIdentity(endpoint, dbName, containerName, partitionKey)`
  - Uses `azidentity.NewDefaultAzureCredential` (workload/Microsoft Entra identity).
  - Creates `azcosmos.NewClient` and container client `client.NewContainer`.

- `NewCosmosDBOrderRepo(endpoint, dbName, containerName, cosmosKey, partitionKey)`
  - Uses `azcosmos.NewKeyCredential` (account key auth).
  - Creates `azcosmos.NewClientWithKey` and container client.

### Data operations
- `GetPendingOrders()` queries `SELECT * FROM o WHERE o.status = @status`.
- `GetOrder(id)` queries `SELECT * FROM o WHERE o.orderId = @orderId` and returns first hit.
- `InsertOrders(orders)` assigns `id` using GUID, sets partition key, and writes each item with `CreateItem`.
- `UpdateOrder(order)` finds existing item by `orderId` and patches `/status` via `PatchItem`.

### Notes
- `GetOrder` returns empty `Order{}` if no match is found; callers may need to handle not-found semantics explicitly.
- `UpdateOrder` assumes document `id` is a string; this may panic in rare malformed scenarios.
- Ensure `ORDER_DB_PARTITION_KEY` and `ORDER_DB_PARTITION_VALUE` are configured to match schema in Cosmos DB container.
